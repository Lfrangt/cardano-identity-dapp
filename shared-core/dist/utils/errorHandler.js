...
exports.isUserCancelledError = exports.isReactDOMError = exports.setupGlobalErrorHandler = void 0;
const setupGlobalErrorHandler = () => {
    // 捕获未处理的错误
    window.addEventListener('error', (event) => {
        const error = event.error;
        // 忽略 React DOM 操作错误
        if (error && (error.message?.includes('insertBefore') ||
    ...
    window.addEventListener('unhandledrejection', (event) => {
        ...
        // 忽略钱包相关的用户取消错误
        if (reason && (reason.message?.includes('User rejected') ||
...
// 检查是否为 React DOM 错误
const isReactDOMError = (error) => {
    return !!(error.message?.includes('insertBefore') ||
        error.message?.includes('removeChild') ||
        error.message?.includes('Node') ||
        error.message?.includes('要删除的节点不是该节点的子节点'));
};
exports.isReactDOMError = isReactDOMError;
// 检查是否为用户取消的钱包操作
