...
exports.packageEncryptedFile = packageEncryptedFile;
exports.unpackageEncryptedFile = unpackageEncryptedFile;
exports.generatePasswordFromWallet = generatePasswordFromWallet;
 ...
 * 将加密数据打包成单个 Blob
 */
function packageEncryptedFile(encryptedData, salt, iv, metadata) {
    // 创建包含所有数据的对象
    const package_data = {
        version: '1.0',
 ...
 * 解包加密文件
 */
function unpackageEncryptedFile(blob) {
    return new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.onload = () => {
            try {
                const package_data = JSON.parse(reader.result);
                resolve({
 ...
 * 这样用户不需要记住额外的密码
 */
async function generatePasswordFromWallet(walletAddress) {
    const encoder = new TextEncoder();
